...
1. `also`：用于执行一个lambda表达式并返回对象本身，跟apply功能一致像，但是采用的是it参数形式传递给Lambda当前对象。
...
- 不允许为`.componentN()`和`.copy()`函数提供显式实现。![null](./img/fxIC8ZAdqzTKDs9.png)
...
原文链接：https://www.itbaima.cn/document/v1zzvki0knb1xvml