...
5. 之后根据以下条件，将年轻代对象复制到老年代空间中：
    1. Survivor已满，则会进行复制
    2. 对象的存活时间达到一定阈值：每执行一次Minor GC，年龄增加1，超过阈值（默认15）后对象会被复制到老年代空间
6. 如果老年代空间满了，则会触发一次 Full GC。若Full GC无法满足程序的内存请求，则很可能触发OOM报错

**老年代空间分配担保规则**

Minor GC前，JVM会比较当前新生代中所有**对象的大小**和老年代**可用空间的大小**

1. 老年代可用空间大小>新生代对象大小，直接触发Minor GC
2. 老年代可用空间大小<新生代对象大小，则比较**老年代可用空间大小**是否大于之前每次Minor GC后**进入老年代的对象的平均大小**
...
1. **标记阶段**：遍历所有的GC Roots对象，对从GC Roots对象可达的对象都打上一个标识（标识一般在对象的header中），将其记录为可达对象
...
1. 标记阶段：与标记-清除算法一致
2. 整理阶段：让所有存活的对象，都向内存的一端移动，然后直接清理掉端边线之外的内存
