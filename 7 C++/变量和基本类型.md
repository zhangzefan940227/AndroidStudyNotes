...
int val = 1000;
int &refVal = val;
cout << val << "  " << refVal << "  " << &refVal << "  " << &val;
// output: 1000  1000  0x48cd7ff7e4  0x48cd7ff7e4
...
```cpp
int ival = 42;
//p1中存放的是ival的地址，也可以说p1是指向变量ival的指针
int* pi = &val;
cout << pi << "  " << *pi;
// output: 0x48cd7ff7e4 1000
...
```cpp
int ival = 42;
int* p = &ival;		//初始化指针p，存放的值为ival对象的地址。
cout << *p << endl;	//这里的*p表示：以p对象中存放的数据为地址，查找该地址对应的对象，然后获取该对象的值，输出42。
```
...
int ival = 42;
int *p = &ival;
int **pi = &p; //pi是一个指向p指针的指针
//输出时，cout ival 等价于 cout *p 等价于 cout **pi
```
