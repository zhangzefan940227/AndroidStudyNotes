...
(function (exports) {
	...
	class StreamReader {
	    constructor(s) {
	        ...
	        this.peekQueue = [];
	        if (!s.read || !s.once) {
	            throw new Error('期望一个stream.Readable的实例');
	        }
	        this.s.once('end', () => this.reject(new EndOfFileStream_1.EndOfStreamError()));
	        this.s.once('error', err => this.reject(err));
	        this.s.once('close', () => this.reject(new Error('流已关闭')));
	    }
	    /**
...
class AbstractTokenizer {
    ...
    normalizeOptions(uint8Array, options) {
        if (options && options.position !== undefined && options.position < this.position) {
            throw new Error('`options.position`必须等于或大于`tokenizer.position`');
        }
        if (options) {
            return {
    ...
    async peekBuffer(uint8Array, options) {
        ...
        if (normOptions.position) {
            ...
            if (skipBytes > 0) {
                ...
                throw new Error('不能从流中的负偏移量进行预览');
            }
        }
        if (normOptions.length > 0) {
            try {
...
async function fromFile$1(sourceFilePath) {
    ...
    if (!stat.isFile) {
        throw new Error(`不是文件: ${sourceFilePath}`);
    }
...
async function fromBuffer(input) {
	if (!(input instanceof Uint8Array || input instanceof ArrayBuffer || Buffer.isBuffer(input))) {
		throw new TypeError(`期望 \`input\` 参数为 \`Uint8Array\` 或 \`Buffer\` 或 \`ArrayBuffer\` 类型，实际得到 \`${typeof input}\` 类型`);
	}
    ...
    {    
        ...
        for(;i<xmlData.length;i++){
            if (xmlData[i] === '<' && !comment) { //Determine the tag type
                ...
                else                                        throw new Error("无效的DOCTYPE");
        ...
        if(angleBracketsCount !== 0){
            throw new Error(`未闭合的DOCTYPE`);
        }
    }else {
        throw new Error(`无效的标签代替了DOCTYPE`);
    }
    return {entities, i};
}
...
function readEntityExp(xmlData,i){
    ...
    if(entityName.indexOf(" ") !== -1) throw new Error("不支持外部实体");
    ...
    if(ch === '<'){
      ...
      if( xmlData[i+1] === '/') {//Closing Tag
        ...
        if(tagName && this.options.unpairedTags.indexOf(tagName) !== -1 ){
          throw new Error(`未配对的标签不能作为结束标签使用: </${tagName}>`);
        }
        let propIndex = 0;
        ...
        if(!tagData) throw new Error("Pi 标签没有关闭。");
        ...
        if (this.isItStopNode(this.options.stopNodes, jPath, tagName)) {
          ...
          //normal tag
            ...
            if(!result) throw new Error(`意外结束的 ${rawTagName}`);
            i = result.i;
...
class XMLParser$2{
    ...
    parse(xmlData,validationOption){
        if(typeof xmlData === "string");else if( xmlData.toString){
            ...
            throw new Error("XML数据接受String或Bytes[]形式。")
        }
        if( validationOption){
    ...
    addEntity(key, value){
        if(value.indexOf("&") !== -1){
            throw new Error("实体值不能包含'&'")
        }else if(key.indexOf("&") !== -1 || key.indexOf(";") !== -1){
            throw new Error("实体必须没有'&'和';'。例如，使用'#xD'代替'&#xD;'。")
        }else if(value === "&"){
            throw new Error("不允许有'&'值的实体");
        }else {
...
function filenamify(string, options = {}) {
	...
	if (filenameReservedRegex().test(replacement) && reControlChars.test(replacement)) {
		throw new Error('替换字符串不能包含保留的文件名字符');
	}
...
function commonjsRequire(path) {
	throw new Error('无法动态加载"' + path + '"。请适当配置@rollup/plugin-commonjs的dynamicRequireTargets或ignoreDynamicRequires选项，以便此require调用可以工作。');
}
...
(function (root, factory) {
			...
			var CryptoJS = CryptoJS || (function (Math, undefined$1) {
			    ...
			    var cryptoSecureRandomInt = function () {
			        ...
			        throw new Error('无法使用原生crypto模块来获取安全随机数。');
			    };
...
function displayError(error, file) {
    ...
    logError(`LocalImagesPlus: 错误: ${error}`, false);
}
...
function replaceAsync(str, regex, asyncFn) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        data.forEach((element) => {
            if (element !== null) {
                logError("元素: " + element[0] + "  元素2: " + element[1] + element[2]);
...
var generator$3 = {
  ...
  Assertion: function Assertion(node) {
    switch (node.kind) {
      ...
      default:
        throw new TypeError('未知断言类型: ' + node.kind);
    }
  },
...
var yyparse = {
  ...
  parse: function parse(string, parseOptions) {
    if (!tokenizer) {
      throw new Error('未指定Tokenizer实例。');
    }
...
function _classCallCheck$6(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("不能将类作为函数调用"); } }
  ...
  _createClass$6(NodePath, [{
    ...
    value: function _enforceProp(property) {
      if (!this.node.hasOwnProperty(property)) {
        throw new Error('类型为 ' + this.node.type + ' 的节点没有 "' + property + '" 集合。');
      }
    }
...
function _classCallCheck$5(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("不能将类作为函数调用"); } }
  ...
  transform: function transform(regexp) {
    ...
    transformToApply.forEach(function (transformName) {
      ...
      if (!compatTransforms.hasOwnProperty(transformName)) {
        throw new Error('未知的兼容转换: ' + transformName + '. 可用的转换有: ' + Object.keys(compatTransforms).join(', '));
      }
...
function disjunctionToList$1(node) {
  if (node.type !== 'Disjunction') {
    throw new TypeError('期望 "Disjunction" 节点，实际得到 "' + node.type + '"');
  }
    ...
    do {
      ...
      transformToApply.forEach(function (transformName) {
        if (!optimizationTransforms.has(transformName)) {
          throw new Error('未知的优化转换: ' + transformName + '. 可用的转换有: ' + Array.from(optimizationTransforms.keys()).join(', '));
        }
...
function _classCallCheck$4(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("不能将类作为函数调用"); } }
...
function _classCallCheck$3(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("不能将类作为函数调用"); } }
...
function _classCallCheck$2(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("不能将类作为函数调用"); } }
...
function _classCallCheck$1(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("不能将类作为函数调用"); } }
...
var generator$1 = {
  RegExp: function RegExp(node) {
    if (node.flags !== '') {
      throw new Error('NFA/DFA: 尚不支持标志。');
    }
  ...
  Repetition: function Repetition(node) {
    switch (node.quantifier.kind) {
      ...
      default:
        throw new Error('未知的重复: ' + node.quantifier.kind + '.');
    }
  },
  Char: function Char(node) {
    if (node.kind !== 'simple') {
      throw new Error('NFA/DFA: 目前只支持简单字符。');
    }
...
class SettingTab extends obsidian.PluginSettingTab {
    ...
    display() {
        ...
        // donheader.createEl("a", { text: "支持该项目！", href: "https://www.buymeacoffee.com/sergeikorneev", cls: "donheader_txt" })
        containerEl.createEl("h3", { text: "界面设置" });
        new obsidian.Setting(containerEl)
            .setName("显示通知")
            .setDesc("当页面处理完毕时显示通知。")
        ...
        new obsidian.Setting(containerEl)
            .setName("禁用附加命令")
            .setDesc("不在命令面板中显示附加命令。在设置中重新加载插件以生效（关闭/开启）。")
            .addToggle((toggle) => toggle
        ...
        new obsidian.Setting(containerEl)
            .setName("自动处理间隔")
            .setDesc("处理更新的时间间隔（秒）。向插件显示笔记更改内容需要一些时间。")
            .addText((text) => text
        ...
        new obsidian.Setting(containerEl)
            .setName("每个附件的重试次数")
            .setDesc("如果下载过程中出现错误（如网络问题），尝试重新下载多次。")
            .addText((text) => text
        ...
        new obsidian.Setting(containerEl)
            .setName("处理所有新的Markdown文件")
            .setDesc("处理所有具有相应扩展名的新创建/云同步文件。")
            .addToggle((toggle) => toggle
        ...
        new obsidian.Setting(containerEl)
            .setName("下载未知文件类型")
            .setDesc("下载未知文件类型并以.unknown扩展名保存它们。")
            .addToggle((toggle) => toggle
        ...
        new obsidian.Setting(containerEl)
            .setName("将PNG转换为JPEG（网络图片）")
            .setDesc("将所有下载的PNG文件转换为JPEG。可能会使文件大小减少几倍，但也可能影响性能。")
            .addToggle((toggle) => toggle
        ...
        new obsidian.Setting(containerEl)
            .setName("将PNG转换为JPEG（粘贴图片）")
            .setDesc("将所有粘贴的PNG文件转换为JPEG。可能会使文件大小减少几倍，但也可能影响性能。")
            .addToggle((toggle) => toggle
        ...
        new obsidian.Setting(containerEl)
            .setName("文件大小下限（KB）")
            .setDesc("不要下载小于这个值的文件。设置为0表示无限制。")
            .addText((text) => text
        ...
        new obsidian.Setting(containerEl)
            .setName("不创建Obsidian附件文件夹（与其他插件兼容）")
            .setDesc("该插件不会创建一个Obsidian附件文件夹。这可能导致插件行为异常。")
            .addToggle((toggle) => toggle
        ...
        containerEl.createEl("h3", { text: "孤立附件" });
        new obsidian.Setting(containerEl)
            .setName("完全移除文件")
            .setDesc("不要将孤立文件移到回收站。")
            .addToggle((toggle) => toggle
        ...
        containerEl.createEl("h3", { text: "媒体文件夹设置" });
        new obsidian.Setting(containerEl)
            .setName("如何在标签中写入路径")
            .setDesc("选择是否在标签中写入完整路径。")
            .addDropdown((text) => text
            .addOption("fullDirPath", "完整路径")
            .addOption("onlyRelative", "相对于笔记")
            .addOption("baseFileName", "仅文件名")
        ...
        new obsidian.Setting(containerEl)
            ...
            .addDropdown((text) => text
            .addOption("obsFolder", "复制Obsidian设置")
            .addOption("inFolderBelow", "在下面指定的根文件夹中")
            .addOption("nextToNoteS", "在下面指定的文件夹中紧邻笔记")
        ...
        containerEl.createEl("h3", { text: "故障排除" });
        new obsidian.Setting(containerEl)
            .setName("调试")
            .setDesc("启用控制台调试输出。")
            .addToggle((toggle) => toggle
...
function chooseFileName(adapter, dir, link, contentData, settings) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        logError("文件: " + link + " 内容: " + contentData + " 文件扩展名: " + fileExt, false);
        ...
        logError("文件名: " + fileName, false);
        if (!fileName) {
            throw new Error("生成媒体文件名失败。");
        }
...
class LocalImagesPlugin extends obsidian.Plugin {
    constructor() {
        ...
        this.processActivePage = (defaultdir = false) => () => __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                showBalloon(`请选择一个笔记或点击画布中选定的笔记。`, this.settings.showNotifications);
                return;
            }
        });
        ...
        this.removeOrphans = (type = undefined, filesToRemove = undefined, noteFile = undefined) => () => __awaiter(this, void 0, void 0, function* () {
            ...
            if (type == "plugin") {
                ...
                if (this.settings.saveAttE != "nextToNoteS" ||
                    ...
                    showBalloon("此命令需要启用设置 '在下面指定文件夹中的笔记旁边' 并且路径末尾有模式 '${notename}'，同时路径不能包含 ${date} 模式。\n请先更改设置！\r\n", this.settings.showNotifications);
                    return;
                }
                if (!noteFile) {
                    noteFile = this.getCurrentNote();
                    if (!noteFile) {
                        showBalloon("请选择一个笔记或点击画布中的笔记！", this.settings.showNotifications);
                        return;
                    }
                }
                if (this.ExemplaryOfMD(noteFile.path)) {
                    ...
                    if (!(yield this.app.vault.exists(oldRootdir))) {
                        showBalloon("附件文件夹 " + oldRootdir + " 不存在！", this.settings.showNotifications);
                        return;
                    }
                    ...
                    if (orphanedAttachments.length > 0) {
                        ...
                        showBalloon("未找到孤立文件！", this.settings.showNotifications);
                    }
                }
            }
            if (type == "obsidian") {
                if (obsmediadir.slice(0, 2) == "./" || obsmediadir == "/") {
                    showBalloon("此命令不能在仓库根目录或笔记旁边的子目录上运行！\n请先更改设置！\r\n", this.settings.showNotifications);
                    return;
                }
                ...
                for (let note of this.noteModified) {
                    ...
                    if (obsmdir != "" && !(yield this.app.vault.adapter.exists(obsmdir))) {
                        if (!this.settings.DoNotCreateObsFolder) {
                            ...
                            showBalloon("您的 Obsidian 媒体文件夹设置为 '" + obsmdir + "'，并且是由插件创建的。请再试一次。", this.settings.showNotifications);
                            onRet();
                        }
                        return;
                    }
                        ...
                        for (let el of embeds) {
                            ...
                            if ((this.newfCreated.indexOf(el.link) != -1 || (obsmdir != "" && (this.newfCreated.includes(oldpath) || this.newfCreated.includes(el.link)))) &&
                                ...
                                if (yield this.app.vault.adapter.exists(newpath)) {
                                    ...
                                    logError(`重命名  ${oldpath}  到  ${newpath}`);
                                    try {
                                        if (newBinData != null) {
            ...
            try {
                ...
                if (title !== null) {
                    ...
                    if (trimAny(noteFile.path, ["\\", "/"]) != trimAny(fullPath, ["\\", "/"])) {
                        ...
                        showBalloon(`笔记已重命名为 ` + fullPath, this.settings.showNotifications);
                    }
                }
            }
            catch (e) {
                showBalloon(`无法重命名。`, this.settings.showNotifications);
                return;
            }
        });
                ...
                if (this.settings.saveAttE !== "obsFolder" &&
                    ...
                    try {
                        if (this.app.vault.getAbstractFileByPath(rootdir) instanceof obsidian.TFolder) {
                            ...
                            showBalloon("附件文件夹 " + rootdir + " 已移动到回收站。", this.settings.showNotifications);
                        }
                    }
                    catch (e) {
                        logError(e);
                ...
                if (path__default["default"].basename(oldRootdir).includes("${notename}") &&
                    ...
                    try {
                        if (this.app.vault.getAbstractFileByPath(oldRootdir_) instanceof obsidian.TFolder) {
                            ...
                            showBalloon("附件文件夹已重命名为 " + newRootDir_, this.settings.showNotifications);
                        }
                    }
                    catch (e) {
                        showBalloon("无法移动附件文件夹: \r\n" + e, this.settings.showNotifications);
                        logError(e);
                        return;
                    }
    ...
    getCurrentNote() {
        try {
            ...
            showBalloon("无法获取当前笔记！", this.settings.showNotifications);
        }
        return null;
    }
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (content != fixedContent[0] && fixedContent[1] === false) {
                ...
                if (this.settings.showNotifications) {
                    showBalloon(`页面 "${file.path}" 已被处理，但没有任何改变。`, this.settings.showNotifications);
                }
            }
        });
    }
                        ...
                        if (this.settings.realTimeUpdate) {
                            ...
                            for (const reg_p of MD_SEARCH_PATTERN) {
                                if (reg_p.test(cont)) {
                                    ...
                                    showBalloon("发现了媒体链接，正在处理...", this.settings.showNotifications);
